...
# Corrected: August 15, 2025 - Clear beacon_last_seen_data on startup to prevent false positives
# Corrected: August 16, 2025, 14:30 UTC - Refactored camera activation logic. main_system_loop now controls START/STOP_CAMERA commands,
#           camera_control_task only processes worker results. Added SCAN_COOLDOWN_SEC to prevent rapid re-triggers.
# Corrected: August 16, 2025, 16:30 UTC - Implemented Blackbox-Prinzip for camera_worker. Removed camera_control_task.
#           main_system_loop now directly orchestrates camera scans and processes results.
#           Removed STOP_SCAN command and SCAN_COOLDOWN_SEC. Adjusted logging prefixes.
...
# Globale Variable für den Zeitpunkt der letzten Türöffnung, für erzwungene Beacon-Abwesenheit
last_door_opened_timestamp = 0 

# Globale Queues für die Inter-Prozess-Kommunikation mit dem Kamera-Worker
...
CODESEND_CODE_BASIS = 1008 # Startcode für 3 Sekunden Öffnungszeit (vom Arduino Sketch) - Default 9128374
...
# Konfiguration für erzwungene Beacon-Abwesenheit nach Türöffnung
FORCE_BEACON_ABSENCE_DURATION_SEC = 10 # Sekunden: Für diese Dauer wird der Beacon nach Türöffnung als abwesend behandelt.
...
async def get_weather_data_async():
    ...
    try:
        logging.info(f"DISP: Frage Wetterdaten von {PWS_QUERY_URL} ab...")
...
def load_icons():
    ...
    try:
        ...
        logging.info(f"DISP: Icons geladen und auf {ICON_DIMENSIONS} / {WEATHER_ICON_SIZE} skaliert.")
...
async def scan_for_ibeacons_task(allowed_users_data):
    ...
    logging.info(f"BLE: Suche nach iBeacon UUID: {TARGET_IBEACON_UUID}")
            ...
            if len(mfg_data) >= 23 and mfg_data[0] == 0x02 and mfg_data[1] == 0x15:
                ...
                if uuid_str == TARGET_IBEACON_UUID and major_val in allowed_majors:
                    ...
                    if distance < PROXIMITY_DISTANCE_THRESHOLD:
                        ...
                        # logging.debug(f"BLE: Beacon {device.address} (Major: {major_val}) erkannt. RSSI: {rssi_val} dBm, Distanz: {distance:.2f}m")
    ...
    while True:
        ...
        for addr in addresses_to_remove:
            ...
            logging.info(f"BLE: Beacon {addr} aus Liste entfernt (zu alt oder zu weit weg).")
...
async def display_manager_task(display_queue):
    ...
    logging.info("DISP: EXTCOMIN Toggling Thread gestartet.")
...
async def main_system_loop():
    global beacon_is_present, last_door_opened_timestamp
    ...
    logging.info(f"MAIN: Lade Gesichts-Encodings von '{ENCODINGS_FILE}'...")
    try:
        with open(ENCODINGS_FILE, 'rb') as f:
            known_face_encodings, known_face_names = pickle.load(f)
        logging.info(f"MAIN: {len(known_face_encodings)} Encodings von {len(set(known_face_names))} Personen geladen.")
    except FileNotFoundError:
        logging.error(f"MAIN: Fehler: Die Datei '{ENCODINGS_FILE}' wurde nicht gefunden. Bitte führen Sie zuerst das Trainingsskript aus.")
        return
    except Exception as e:
        logging.error(f"MAIN: Fehler beim Laden der Encodings: {e}")
        return

    # Lese die erlaubten Nutzer und Beacon-Majors aus der Konfigurationsdatei
    allowed_users_data = read_allowed_users_config()
    logging.info(f"MAIN: Erlaubte Nutzer aus '{ALLOWED_USERS_CONFIG}' geladen: {allowed_users_data}")

    # Erstelle die major_to_names_map (Major-Wert -> Liste von Namen)
    major_to_names_map = {}
    ...
    logging.info(f"MAIN: Erstellte Major-zu-Namen-Zuordnung: {major_to_names_map}")

    # Starte den Kamera-Worker-Prozess
    initial_worker_config = {
        ...
        'max_scan_duration_sec': 15 # Maximale Scan-Dauer für den Worker
    }
    ...
    logging.info(f"MAIN: Kamera-Worker-Prozess gestartet mit PID: {camera_worker_process.pid}")
    ...
    is_scan_active_in_worker = False # Interner Zustand: Hat der Hauptprozess einen Scan angefordert und wartet auf Ergebnis?
    ...
    try:
        while True:
            ...
            # Erzwinge Beacon-Abwesenheit nach Türöffnung
            if (current_time - last_door_opened_timestamp) < FORCE_BEACON_ABSENCE_DURATION_SEC:
                if any_relevant_beacon_in_proximity: # Nur loggen, wenn es tatsächlich einen Beacon gab
                    logging.info(f"MAIN: Erzwinge Beacon-Abwesenheit für {FORCE_BEACON_ABSENCE_DURATION_SEC}s (Cooldown nach Türöffnung).")
                any_relevant_beacon_in_proximity = False # Zwinge den Status auf False
            ...
            if current_beacon_state_raw and not beacon_is_present: # Beacon ist roh da, aber noch nicht stabil present
                if (current_time - last_beacon_state_change_time) >= PRESENCE_DETECTION_TIME:
                    ...
                    if not is_scan_active_in_worker: # Nur starten, wenn kein Scan läuft
                        if (current_time - last_door_opened_timestamp) >= FORCE_BEACON_ABSENCE_DURATION_SEC:
                            logging.info("MAIN: Sende PERFORM_SCAN Befehl an Worker.")
                            camera_command_queue.put({"command": "PERFORM_SCAN", "expected_names": list(current_expected_names)})
                            is_scan_active_in_worker = True
                            await display_status_queue.put({"type": "status", "value": "BEACON_DETECTED", "duration": 5})
                        else:
                            logging.info(f"MAIN: Kamera-Scan nicht initiiert: Cooldown nach Türöffnung aktiv (noch {FORCE_BEACON_ABSENCE_DURATION_SEC - (current_time - last_door_opened_timestamp):.1f}s).")
                    else:
                        logging.info("MAIN: Kamera-Scan bereits aktiv, warte auf Ergebnis.")
                ...
                if (current_time - last_beacon_state_change_time) >= ABSENCE_DETECTION_TIME:
                    ...
                    # Der Worker stoppt die Kamera selbst nach Match/Timeout. Kein STOP_SCAN Befehl hier.

            # NEU: Prüfe auf Ergebnisse vom Kamera-Worker (nicht-blockierend)
            try:
                result_message = camera_result_queue.get_nowait()
                logging.info(f"MAIN: Ergebnis vom Worker: {result_message}")
                
                if result_message.get("status") == "FACE_MATCH_CONFIRMED":
                    logging.info(f"MAIN: Gesichtsmatch vom Worker bestätigt für {result_message.get('name')}. Öffne Tür.")
                    ...
                    last_door_opened_timestamp = time.time() # Tür geöffnet, Cooldown starten
                    is_scan_active_in_worker = False # Scan abgeschlossen
                    
                elif result_message.get("status") == "NO_FACE_MATCH":
                    logging.info(f"MAIN: Kein Gesichtsmatch vom Worker. Grund: {result_message.get('reason')}.")
                    # Hier könnte man ein "Access Denied" Icon senden, falls gewünscht
                    # await display_status_queue.put({"type": "status", "value": "ACCESS_DENIED", "duration": 3})
                    
                    is_scan_active_in_worker = False # Scan abgeschlossen

            except multiprocessing.queues.Empty:
                pass # Keine neue Nachricht vom Worker
        ...
        logging.info("MAIN: Sende TERMINATE Befehl an Kamera-Worker-Prozess.")
        ...
        if camera_worker_process.is_alive():
            logging.warning("MAIN: Kamera-Worker-Prozess konnte nicht sauber beendet werden. Terminiere ihn.")
            camera_worker_process.terminate()
        logging.info("MAIN: Kamera-Worker-Prozess beendet.")
        ...
        try:
            with open(ALLOWED_USERS_CONFIG, 'r') as f:
                for line in f:
                    ...
                    parts = line.split(';')
                    if len(parts) < 2:
                        logging.warning(f"MAIN: Ungültige Zeile in '{ALLOWED_USERS_CONFIG}': '{line}'. Erwarte mindestens Name;Status.")
                        continue
                    ...
                    for i in range(2, len(parts)):
                        ...
                        if major_str:
                            try:
                                ...
                                logging.warning(f"MAIN: Ungültiger Major-Wert '{major_str}' für Nutzer '{name}' in '{ALLOWED_USERS_CONFIG}'. Ignoriere.")
            ...
            logging.error(f"MAIN: Fehler beim Lesen von '{ALLOWED_USERS_CONFIG}': {e}")
