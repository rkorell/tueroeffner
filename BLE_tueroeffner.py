...
# Modified: October 10, 2025, 13:45 UTC - Initialisierung von 'is_currently_inside_house' und Implementierung des dedizierten Initial-Scans.
...
INITIAL_SCAN_DURATION_SEC = 15 # NEU: Default für Initial Scan
BEACON_ABSENCE_TIMEOUT_FOR_HOME_STATUS_SEC = 3600 # NEU: Default für Home Status Timeout
...
#                    "is_currently_inside_house": bool } } # NEU: is_currently_inside_house
...
# --- NEU: Callback für den Initial-Scan ---
def initial_scan_detection_callback(device, advertisement_data):
    ...
    beacon_mp = beacon_config.get("calibrated_measured_power", SYSTEM_CONFIG["system_globals"]["calibrated_measured_power_global_default"])
    beacon_n = beacon_config.get("path_loss_exponent", SYSTEM_CONFIG["system_globals"]["path_loss_exponent_global_default"])
    ...
    beacon_last_seen_data[current_mac] = {
        'timestamp': time.time(),
...
# --- NEU: Funktion für den dedizierten Initial-Scan ---
async def _perform_initial_beacon_scan(duration_sec):
    logging.info(f"MAIN: Starte dedizierten Initial-Scan für {duration_sec} Sekunden...")
    ...
    logging.info("MAIN: Initial-Scan beendet.")
...
async def scan_for_ibeacons_task():
    ...
    def detection_callback(device, advertisement_data):
        if TRACE_MODE:
            t0 = perf_counter()
            # logging.info("TRACE: +%7.3f ms  Start detection_callback MAC=%s", 0.0, device.address)

        current_mac = device.address

        # --- KORREKTUR: Initialisiere Variablen zu Beginn ---
        parsed_ibeacon = None
        ...
        if not beacon_state['is_fully_identified']:
            ...
            if is_match:
                ...
                if TRACE_MODE and beacon_state.get('is_fully_identified'):
                    logging.info(
        ...
        # --- KORREKTUR: Verwende beacon-spezifische Kalibrierungswerte für Distanzschätzung ---
        beacon_mp = beacon_config.get("calibrated_measured_power", SYSTEM_CONFIG["system_globals"]["calibrated_measured_power_global_default"])
        beacon_n = beacon_config.get("path_loss_exponent", SYSTEM_CONFIG["system_globals"]["path_loss_exponent_global_default"])
        distance = estimate_distance(advertisement_data.rssi, beacon_mp, beacon_n)
    ...
    try:
        while True: # Changed from while ble_scan_active to allow graceful shutdown
            ...
            for mac in beacons_to_remove:
                ...
                for cfg in SYSTEM_CONFIG["known_beacons"]:
                    if cfg.get("mac_address") == mac:
                        original_beacon_config = cfg
                        break

                if mac in beacon_identification_state: # Prüfen, ob der MAC noch existiert
                    del beacon_identification_state[mac] # Entfernen des alten Eintrags
...
async def main():
    ...
    for beacon_cfg in SYSTEM_CONFIG["known_beacons"]:
        ...
        if mac_addr:
            ...
            beacon_last_seen_data[mac_addr] = {
    ...
    # --- NEU: Dedizierter Initial-Scan beim Systemstart ---
    await _perform_initial_beacon_scan(SYSTEM_CONFIG["system_globals"]["initial_scan_duration_sec"])
    ...
    for mac, state in beacon_identification_state.items():
        ...
        if state["is_allowed"] and state["is_currently_inside_house"]:
            ...
            if mac not in beacon_last_seen_data or \
               beacon_last_seen_data[mac]['timestamp'] < (initial_scan_end_time - SYSTEM_CONFIG["system_globals"]["initial_scan_duration_sec"]):
        ...
        while True:
            ...
            for mac, state in beacon_identification_state.items():
                ...
                if state["is_currently_inside_house"] and \
                   (current_time - state['last_packet_time']) > SYSTEM_CONFIG["system_globals"]["beacon_absence_timeout_for_home_status_sec"]:
                    state["is_currently_inside_house"] = False
            ...
            for mac in beacons_to_remove:
                ...
                if mac in beacon_identification_state: # Prüfen, ob der MAC noch existiert
                    ...
                    if mac in beacon_last_seen_data:
                        del beacon_last_seen_data[mac]
            ...
            # --- NEU: Logik für Distanz-Debouncing und Türöffnung ---
            # 'any_relevant_beacon_in_proximity' bleibt hier für die allgemeine 'beacon_is_present' Logik,
            # hat aber keine direkte Auswirkung auf die Türöffnung selbst.

            for mac, state in beacon_identification_state.items():
                # 1. Proximity-Status für jeden Beacon aktualisieren
                beacon_data = beacon_last_seen_data.get(mac)
                ...
                current_is_in_proximity_raw = (current_distance != -1.0 and current_distance <= SYSTEM_CONFIG["system_globals"]["proximity_distance_threshold"])
                ...
                if current_is_in_proximity_raw and not state['is_in_proximity_debounced']:
                    if (current_time - state['proximity_state_change_time']) >= SYSTEM_CONFIG["system_globals"]["presence_detection_time"]:
                    ...
                    if (current_time - state['proximity_state_change_time']) >= SYSTEM_CONFIG["system_globals"]["absence_detection_time"]:
                ...
                if state['is_fully_identified'] and \
                   ...
                   (current_time - last_door_opened_timestamp) > SYSTEM_CONFIG["system_globals"]["force_beacon_absence_duration_sec"]:
                    ...
                    last_door_opened_timestamp = current_time # Cooldown für Türöffnung starten
            ...
            # --- ENDE NEU: Logik für Distanz-Debouncing und Türöffnung ---
