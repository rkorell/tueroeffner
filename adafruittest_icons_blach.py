...
WEATHER_ICON_SIZE = (24, 24) # NEU: Größe für Wind- und Regen-Icons
...
def load_icons():
    ...
    try:
        ...
        ICON_EYE = ImageOps.invert(ICON_EYE) # Icons invertieren, damit sie schwarz auf weißem Grund sind

        key_img = Image.open(key_path).resize(ICON_DIMENSIONS, Image.LANCZOS)
        ICON_KEY = key_img.convert('1') # 1-Bit (Schwarz/Weiß)
        ICON_KEY = ImageOps.invert(ICON_KEY) # Icons invertieren

        # NEU: Wind-Icon laden
        wind_img = Image.open(wind_path).resize(WEATHER_ICON_SIZE, Image.LANCZOS)
        ICON_WIND = wind_img.convert('1')
        ICON_WIND = ImageOps.invert(ICON_WIND)

        # NEU: Regen-Icon laden
        rain_img = Image.open(rain_path).resize(WEATHER_ICON_SIZE, Image.LANCZOS)
        ICON_RAIN = rain_img.convert('1')
        ICON_RAIN = ImageOps.invert(ICON_RAIN)
...
def draw_display_content(draw, weather_data, status_icon_type=None):
    ...
    if weather_data:
        ...
        if ICON_WIND is not None:
            draw.bitmap((5, 110), ICON_WIND, fill=BLACK) # Icon linksbündig
            draw.text((5 + WEATHER_ICON_SIZE[0] + 5, 110 + (WEATHER_ICON_SIZE[1] - FONT_WEATHER_DETAIL.getbbox('')[3]) // 2), # Text rechts vom Icon, vertikal zentriert
                      f"{weather_data.get('wind_speed', 'N/A')} {weather_data.get('wind_direction', 'N/A')}", font=FONT_WEATHER_DETAIL, fill=BLACK)
        else:
            draw.text((5, 110), f"Wind: {weather_data.get('wind_speed', 'N/A')} {weather_data.get('wind_direction', 'N/A')}", font=FONT_WEATHER_DETAIL, fill=BLACK)
        
        # NEU: Regen-Icon und Text
        if ICON_RAIN is not None:
            draw.bitmap((5, 135), ICON_RAIN, fill=BLACK) # Icon linksbündig
            draw.text((5 + WEATHER_ICON_SIZE[0] + 5, 135 + (WEATHER_ICON_SIZE[1] - FONT_WEATHER_DETAIL.getbbox('')[3]) // 2), # Text rechts vom Icon, vertikal zentriert
                      f"{weather_data.get('precipitation', 'N/A')}", font=FONT_WEATHER_DETAIL, fill=BLACK)
        else:
            draw.text((5, 135), f"Regen: {weather_data.get('precipitation', 'N/A')} ", font=FONT_WEATHER_DETAIL, fill=BLACK)
