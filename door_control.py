...
# Modified: November 07, 2025, 14:42 UTC - Logging-Refactor: Benannter Logger, Präfixe entfernt.
...
# NEU: Benannter Logger (Phase 3.4)
...
async def send_door_open_command(duration_sec):
    ...
    if (current_time - gs._last_codesend_time) < min_detection_interval:
        log.info(f"codesend Cooldown aktiv. Nächste Auslösung in {min_detection_interval - (current_time - gs._last_codesend_time):.1f}s.")
        return
    ...
    if not (codesend_min_duration_sec <= duration_sec <= 10):
        log.error(f"Ungültige Dauer für codesend: {duration_sec} Sekunden. Muss zwischen {codesend_min_duration_sec} und 10 Sekunden liegen.")
        return

    code_to_send = codesend_code_basis + (duration_sec - codesend_min_duration_sec)
    
    try:
        log.info(f"Sende Türöffner-Befehl: codesend {code_to_send}")
        ...
        log.info(f"codesend erfolgreich aufgerufen. Output: {process.stdout.strip()}")
        gs._last_codesend_time = current_time
    except FileNotFoundError:
        log.error(f"Fehler: codesend nicht gefunden unter {codesend_path}. Bitte Pfad prüfen.")
    except subprocess.CalledProcessError as e:
        log.error(f"Fehler beim Aufruf von codesend: {e}. Stdout: {e.stdout}, Stderr: {e.stderr}")
    except Exception as e:
        log.error(f"Ein unerwarteter Fehler beim Senden des codesend-Befehls ist aufgetreten: {e}")